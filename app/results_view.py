...
def render_results_section(combined_candidates, memory_column, updated_by, tanggal_proses=None):
    ...
    preview_columns = [
        ...
        "Status Justifikasi", "PIC Owner", "Status HK", "Catatan"
    ]
    preview_columns = [
    ...
    st.markdown(
        ...
        "Anda dapat mengubah `PIC Owner`, `Status HK`, dan `Catatan` langsung pada tabel."
    )
    ...
    edited_dataframe = st.data_editor(
        ...
        column_config={
            ...
            "Status HK": st.column_config.SelectboxColumn(
                ...
                width="medium",
                options=VALID_STATUSES, # Ambil dari status_tracking.py (Need Confirm, dll)
                required=True,
            ),
    ...
    if st.button("💾 Simpan Perubahan", type="primary"):
        if not updated_by or updated_by.strip() == "":
            ...
            changed_rows = edited_dataframe[
                ...
                | (edited_dataframe["PIC Owner"] != display_dataframe["PIC Owner"])
            ]
            if changed_rows.empty:
                st.toast("Tidak ada perubahan untuk disimpan.", icon="ℹ️")
            else:
                save_status_updates(
                    display_dataframe,
                    edited_dataframe,
